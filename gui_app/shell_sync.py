...
class ShellSyncMixin(ShellSyncUiMixin, ShellSyncPublishMixin):
        ...
        if should_prompt_first_run(sett, db_path):
            try:
                yes = messagebox.askyesno(
                    "Public database",
                    "Download the shared public arrest database from GitHub?\n\n"
                    "• Public arrest / booking fields only (no private user data)\n"
                    "• Includes archived mugshot photos (may be large)\n"
                    "• Paths are project-relative (no local user folders)\n"
                    "• Your local ethnicity classifications are preserved on update\n"
    ...
    def _run_db_sync_background(
        ...
        *,
        ...
        repo = str(sett.get("db_sync_repo") or "HyperboreanSlug/MAPA")
        ...
        def worker() -> None:
            ...
            def log(m: str) -> None:
                try:
                    self.log(f"DB sync ({reason or 'manual'}): {m}")
                except Exception:
                    pass
            ...
            def done() -> None:
                ...
                if err:
                    ...
                    try:
                        self.log(f"DB sync: {result.message}")
                    except Exception:
                        pass
