...
def draw_race_banner(
    ...
    label = "Reported As"
    race_txt = (race or "Unknown").upper().strip()
    ...
    _draw_race_value_wide(
        draw, race_line, race_font, ban_left + 24, ban_right - 24, cy - rb[1],
        fill=_BANNER_TEXT,
    )
    if len(race_lines) > 1:
        cy += rh + gap
        rb2 = draw.textbbox((0, 0), race_lines[1], font=race_font)
        _draw_race_value_wide(
            draw, race_lines[1], race_font, ban_left + 24, ban_right - 24,
            cy - rb2[1], fill=_BANNER_TEXT,
        )
    return top + BANNER_H


def _draw_race_value_wide(
    draw, text: str, font, left: int, right: int, y: int, *, fill,
) -> None:
