"""Apply public delta ops onto a local arrests SQLite database."""
...
from scraper.db_sync_preserve import load_local_row_overlay, merge_overlay_into_row
...
def build_key_index(conn: sqlite3.Connection) -> Dict[str, List[int]]:
    """Map sync key → list of arrest ids (usually one)."""
            ...
            try:
                ...
                if kind == "delete":
                    for rid in idx.pop(key, []):
                        cur.execute("DELETE FROM arrests WHERE id=?", (rid,))
                    n_del += 1
                elif kind == "upsert":
                    row = op.get("row")
                    if not isinstance(row, dict):
                        ...
                        continue
                    old_ids = idx.pop(key, [])
                    overlay = load_local_row_overlay(conn, old_ids)
                    merged = merge_overlay_into_row(row, overlay)
                    for rid in old_ids:
                        cur.execute("DELETE FROM arrests WHERE id=?", (rid,))
                    cur.execute(_INSERT_SQL, _row_tuple(merged))
