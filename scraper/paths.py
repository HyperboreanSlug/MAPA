...
GUI and CLI must resolve ``data/arrests.db`` against the *install root*,
...
DEFAULT_DB_REL = "data/arrests.db"
DEFAULT_SETTINGS_REL = "data/app_settings.json"


def project_root() -> Path:
    """Directory that contains ``data/``, ``gui.py``, ``scraper/``."""
    env = (os.environ.get("MAPA_ROOT") or os.environ.get("ARCHIVER_ROOT") or "").strip()
    if env:
        try:
...
def sanitize_db_path(
    ...
    *,
    ...
    ``C:\\Users\\Other\\SORPA\\data\\arrests.db`` on another machine would
